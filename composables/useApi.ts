...
export const useApi = () => {
  ...
  const reserveMenu = async ({ menuId, options, totalPrice }: { menuId: string; options: MealOptions; totalPrice: number }) => {
  ...
  const confirmTossPayment = async ({ paymentKey, orderId, amount }: { paymentKey: string; orderId: string; amount: number }) => {
    const { data, error } = await supabase.functions.invoke('confirm-toss-payment', { body: { paymentKey, orderId, amount } })
    if (error) throw new Error(error.message)
