...
<template>
  <div
    class="border border-[#eee] rounded-[15px] p-[15px] bg-white shadow-[0_2px_8px_rgba(0,0,0,0.05)] flex flex-col h-full"
  >
    ...
    <div v-if="menu.deposit_amount" class="text-[#777] text-[11px] font-medium mb-[10px]">
      {{ menu.deposit_amount.toLocaleString() }}P 예약금 포함
    </div>

    <div class="mt-auto">
      <select
        v-model="selectedRice"
        class="w-full p-[10px] mt-[5px] rounded-[8px] border border-[#ddd] text-[13px] bg-[#fafafa] focus:outline-none focus:border-[#2E7D32] cursor-pointer"
      >
