...
const send = async () => {
  ...
  try {
    ...
    const assistant = messages.value[messages.value.length - 1]
    if (assistant?.role === 'assistant') assistant.content = error instanceof Error ? error.message : t('chat.errors.default')
  } finally {
    loading.value = false
    await scrollToBottom()
  }
...
<template>
  ...
  <div class="fixed bottom-24 right-4 z-40 md:bottom-7 md:right-7">
    <Transition name="chat-panel">
      <section v-if="isOpen" class="absolute bottom-16 right-0 flex h-[min(560px,70vh)] w-[min(380px,calc(100vw-2rem))] flex-col overflow-hidden rounded-3xl border border-green-100 bg-white shadow-2xl" @click.stop>
        <header class="flex items-center justify-between bg-[#2E7D32] px-5 py-4 text-white">
          ...
          <div class="flex gap-1">
            <button @click="resetConversation" :title="t('chat.new_conversation')" class="rounded-lg px-2 py-1 text-sm hover:bg-white/15">↻</button>
            <button @click="isOpen = false" :title="t('chat.close')" class="rounded-lg px-2 py-1 text-lg hover:bg-white/15">×</button>
          </div>
        </header>
        ...
        <div ref="chatBody" class="flex-1 space-y-3 overflow-y-auto bg-[#F8FAF8] p-4">
          ...
          <article v-for="(item, index) in messages" :key="index" class="flex" :class="item.role === 'user' ? 'justify-end' : 'justify-start'">
            <div class="max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed" :class="item.role === 'user' ? 'rounded-br-md bg-[#2E7D32] text-white whitespace-pre-wrap' : 'rounded-bl-md bg-white text-gray-700 shadow-sm'">
              ...
              <span v-else-if="loading" class="inline-flex gap-1"><i class="h-1.5 w-1.5 animate-bounce rounded-full bg-gray-400" /><i class="h-1.5 w-1.5 animate-bounce rounded-full bg-gray-400 [animation-delay:150ms]" /><i class="h-1.5 w-1.5 animate-bounce rounded-full bg-gray-400 [animation-delay:300ms]" /></span>
            </div>
          </article>
        </div>
        ...
        <form @submit.prevent="send" class="border-t border-gray-100 bg-white p-3">
          <div class="flex items-end gap-2 rounded-2xl border border-gray-200 bg-gray-50 px-3 py-2 focus-within:border-[#2E7D32]">
            <textarea v-model="input" :disabled="loading" :maxlength="100" rows="1" :placeholder="t('chat.placeholder')" class="max-h-24 flex-1 resize-none bg-transparent text-sm outline-none" @compositionend="submitAfterCommittingComposition" @keydown.enter.exact="submitOnEnter" />
    ...
    <button @click="isOpen ? isOpen = false : openChat()" class="flex h-14 w-14 items-center justify-center rounded-full bg-[#2E7D32] text-2xl text-white shadow-lg shadow-green-700/30 transition hover:scale-105" :aria-label="t('chat.title')">💬</button>
  </div>
</template>

<style scoped>
.chat-panel-enter-active, .chat-panel-leave-active { transition: all .2s ease; }
.chat-panel-enter-from, .chat-panel-leave-to { opacity: 0; transform: translateY(12px) scale(.97); }
.chat-markdown :deep(strong) { font-weight: 800; }
.chat-markdown :deep(code) { border-radius: .25rem; background: #f3f4f6; padding: .1rem .25rem; font-size: .8em; }
.chat-list-item { padding-left: .15rem; }
</style>
