...
레거시 SQL·Edge Function은 당시 Neon 데이터 이관과 장애 복구 대조를 위해서만 보존했고 런타임에서는 사용하지 않았다. 이후 Neon 백업 복원 검증을 마치고 해당 원본과 일회성 ETL 도구를 저장소에서 제거했다. 외부 기능을 실제로 사용하려면 로컬 `.env`에 `TOSS_PAYMENTS_SECRET_KEY`, `GEMINI_API_KEY`를 설정해야 한다.
...
## 최종 종료 — 실행 코드와 마이그레이션 기록을 구분한 문제

### 문제

운영 런타임의 직접 호출은 이미 사라졌지만 저장소에는 데이터 이관에 사용한 SQL·Edge Function, Django ETL 명령, 원본 연결 환경 변수 예시가 남아 있었다. 반대로 문자열을 저장소 전체에서 무조건 지우면 포트폴리오에서 중요한 전환 근거와 트러블슈팅 기록까지 사라진다.

### 해결

실행 가능한 경계를 `frontend`, `backend`, `packages`, `infra`, CI workflow와 패키지 lockfile로 정의했다. 이 경계에서는 레거시 디렉터리·SDK·연결 문자열·호출 코드를 모두 제거하고 CI 검색 가드로 재유입을 막았다. 문서에는 이전 구조와 ETL 대조 결과를 완료된 과거형 기록으로 유지하고, 실행할 수 없는 과거 명령은 컷오버 기록으로 축약했다.

### 배운 점

기술 의존성 제거는 패키지 삭제만이 아니라 설정, 운영 스크립트, 일회성 도구와 재유입 방지까지 포함한다. 동시에 역사적 기록과 현재 실행 계약을 분리하면 저장소를 깨끗하게 유지하면서도 마이그레이션 과정의 설계 근거를 보존할 수 있다.
