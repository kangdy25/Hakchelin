...
const handleCancel = async (reservation: Reservation) => {
  ...
  try {
    ...
    await showAlert(t("payment.cancel_error") + ": " + api.getErrorMessage(err), {
      title: t("payment.cancel_error"),
      type: "error"
    });
  } finally {
...
<template>
  <div>
    <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-3 mb-6">
      ...
      <label
        class="flex items-center gap-2.5 cursor-pointer select-none text-sm text-gray-600 font-semibold bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-full hover:bg-gray-100/80 transition-all duration-200 w-fit"
      >
        <input type="checkbox" v-model="hideCancelled" class="sr-only peer" />
        <div
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-[15px]">
      ...
      >
        ...
        >
          <div
            class="border-2 border-dashed border-red-500/40 text-red-500/70 font-black text-[15px] uppercase tracking-widest px-4 py-1.5 rounded-[6px] rotate-[-8deg] bg-white/90 shadow-sm"
          >
            {{ t("status.cancelled") }}
          </div>
        </div>
        ...
        <div class="flex items-start justify-between gap-3 mb-4">
          <div>
            <div class="text-[12px] text-[#777] font-bold mb-1">
              {{ reservation.meal_date || formatDate(reservation.created_at || "") }}
              {{ reservation.meal_time?.slice(0, 5) || "" }}
            </div>
            <h2 class="text-[18px] font-black text-gray-800 leading-snug">{{ menuTitle(reservation) }}</h2>
          </div>
          <span
            class="shrink-0 text-[12px] font-bold px-3 py-1.5 rounded-lg"
            :class="statusClass[reservation.status || 'reserved']"
          >
