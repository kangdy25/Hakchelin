...
<template>
  <div>
    ...
    <div
      class="bg-white rounded-[2rem] p-8 text-center shadow-[0_2px_8px_rgba(0,0,0,0.05)] border border-[#eee] flex flex-col items-center justify-center min-h-[50vh]"
    >
      <div
        class="w-24 h-24 bg-gradient-to-tr from-[#4ade80] to-[#22c55e] rounded-full flex items-center justify-center text-4xl text-white mb-6 shadow-lg shadow-green-200"
      >
        {{ profile.name?.[0] || "👤" }}
      </div>
        ...
        <span
          ...
          :class="
            isAdmin
              ...
              : 'bg-green-50 text-[#2E7D32] border-green-200 shadow-green-100'
          "
        >
          <span>{{ isAdmin ? "👑" : "🎓" }}</span>
      ...
      <div v-if="!loadingStats" class="grid grid-cols-2 gap-4 w-full max-w-sm mt-6 mb-8">
        <div class="bg-[#E8F5E9] border border-green-100 rounded-2xl p-4 text-center">
          <div class="text-[11px] text-[#2E7D32] font-black uppercase tracking-wider mb-1">
            💰 {{ locale === "ko" ? "사전예약 절약" : "Pre-order Saved" }}
          </div>
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-2xl p-4 text-center">
          <div class="text-[11px] text-blue-700 font-black uppercase tracking-wider mb-1">
            🌱 No-Waste {{ locale === "ko" ? "실천" : "Eco Count" }}
          </div>
